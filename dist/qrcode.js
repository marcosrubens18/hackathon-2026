/*
 * QR Code generator — vendored, minimal, dependency-free.
 * Adapted from "qrcode-generator" by Kazuhiko Arase, MIT licensed.
 * https://github.com/kazuhikoarase/qrcode-generator
 *
 * Kept local (no CDN, no network call) so the demonstration works fully
 * offline, matching the rest of WedTech's "sem dependências" prototype.
 * Only what WedTech needs is included: byte-mode encoding, error
 * correction level M, automatic version selection up to 10.
 */
(function (global) {
  "use strict";
  ...
  var QRUtil = (function () {
    ...
    function getMask(maskPattern, i, j) {
      switch (maskPattern) {
        ...
        default: throw Error("bad maskPattern:" + maskPattern);
      }
    }
  ...
  function QRPolynomial(num, shift) {
    if (num.length === undefined) throw Error(num.length + "/" + shift);
    var offset = 0;
  ...
  var QRRSBlock = (function () {
    ...
    return {
      getRSBlocks: function (typeNumber, errorCorrectLevel) {
        ...
        if (!rsBlock) throw Error("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectLevel:" + errorCorrectLevel);
  ...
  function QR8bitByte(data) {
    ...
    this.data = data;
    // UTF-8 encode so accented characters (product names, prices) round-trip correctly.
    this.bytes = (function () {
  ...
  QRCodeModel.prototype = {
    ...
    isDark: function (row, col) {
      if (row < 0 || this.moduleCount <= row || col < 0 || this.moduleCount <= col) throw Error(row + "," + col);
  ...
  // Picks the smallest version (1..10) that fits the data at error-correct level M.
  function createQrCode(data, errorCorrectLevel) {
    var level = errorCorrectLevel === undefined ? QRErrorCorrectLevel.M : errorCorrectLevel;
    for (var typeNumber = 1; typeNumber <= 10; typeNumber++) {
      try {
        ...
        if (typeNumber === 10) throw e;
      }
    }
  }

  // Renders a QRCodeModel as a self-contained <svg> string, `size` px square,
  // with a `margin` of quiet-zone modules (4 is the spec minimum).
  function toSVG(qr, size, margin) {
    var count = qr.getModuleCount();
    var quiet = margin === undefined ? 4 : margin;
    ...
    for (var row = 0; row < count; row++) {
      for (var col = 0; col < count; col++) {
        if (qr.isDark(row, col)) {
          var x = (col + quiet) * cell;
          var y = (row + quiet) * cell;
  ...
  var api = { createQrCode: createQrCode, toSVG: toSVG, QRErrorCorrectLevel: QRErrorCorrectLevel };
  if (typeof module !== "undefined") module.exports = api;
  global.WedTechQR = api;
